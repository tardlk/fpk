...
- **参考**: `apps/moviepilot/`（Python Docker）
- **特点**:
...
### Docker 应用向导变量传递 ⚠️ 重要

Docker 应用在安装向导中收集的用户输入（如密码、路径等）需要传递到容器。**关键原则**：

**不要使用 `env_file` 指令。** fnOS 并非通过 `docker compose` CLI 管理容器，因此 docker-compose.yaml 中的 `env_file` 指令会被忽略。`.env` 文件写入的值永远无法到达容器。

**正确做法：利用 compose 变量替换。** fnOS 在创建容器前会对 docker-compose.yaml 进行环境变量替换（官方文档："`docker-compose.yaml` 允许使用环境变量，在执行前将进行替换"）。安装向导中用户填写的内容会自动成为环境变量，可直接在 compose 文件中使用 `${变量名:-默认值}` 语法引用。

```yaml
# 正确示例：${wizard_password} 由安装向导提供
environment:
  - SUPERUSER_PASSWORD=${wizard_password:-password}

# 错误示例：env_file 在 fnOS 上不生效
# env_file:
#   - .env
```

注意事项：
- 向导变量无 `TRIM_` 前缀，直接使用 wizard 中定义的 `field` 名
- `:-默认值` 语法在用户跳过向导时生效
- 变量值中避免出现 `$`、`{`、`}`、`#`、`"` 等特殊字符，可能破坏 compose 文件解析
- 若变量出现在 compose 顶层（如 `${wizard_port}:3000`），fnOS 同样会替换

对于无法用变量替换覆盖的场景（如 volumes 中的路径绑定部分），在 `service-setup` 的 `service_postinst` 中使用 sed 直接修改 compose 文件。参考 `apps/moviepilot/`。
