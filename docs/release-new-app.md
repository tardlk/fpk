...
| `cmd/main` | Docker 应用 | Docker 应用需要，用于状态检查（与其他 Docker 应用保持一致） |
...
```bash
# 查看 CI 运行状态
gh run list --repo tardlk/fpk --limit 5

# 查看 Release
gh release view <slug>/v<version> --repo tardlk/fpk
...
```bash
gh issue comment <number> --repo tardlk/fpk --body "已发布 ✅ ..."
gh issue close <number> --repo tardlk/fpk --reason completed
```
...
如需手动触发构建：
```bash
gh workflow run "Build <DisplayName> fnOS Package" --repo tardlk/fpk
```
